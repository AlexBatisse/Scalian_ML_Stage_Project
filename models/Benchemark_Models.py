...
def benchmark_models(df: pd.DataFrame, include_lstm: bool = False) -> dict:
    """Entraîne plusieurs modèles sur le même split et les compare.
 
    Retourne les artefacts du MEILLEUR modèle (R² le plus élevé),
    pour l'utiliser ensuite dans les scénarios what-if.
 
    Parameters
    ----------
    df : DataFrame pré-traité
    include_lstm : bool
        Si True, inclut un LSTM (nécessite tensorflow, ~5 min d'entraînement).
    """
    print(f"\n{'='*60}\nBENCHMARK DE MODÈLES\n{'='*60}")
 
    # --- Préparation des données (même split pour tous) ---
    ...
    d_tr = (df_train['date'].max() - df_train['date'].min()).days
    d_te = (df_test ['date'].max() - df_test ['date'].min()).days
    print(f"Train : {len(df_train):,} obs ≈ {d_tr} j | Test : {len(df_test):,} obs ≈ {d_te} j")
 
    # --- Définition des modèles à comparer ---
    models_config = {
        'Ridge (régression linéaire)': Ridge(alpha=1.0, random_state=RANDOM_STATE),
    ...
    results = {}
    print(f"\n{'Modèle':<30s} {'MAE':>8s} {'RMSE':>8s} {'R²':>8s} {'Temps':>8s}")
    print("-" * 66)
 
    for name, model in models_config.items():
        t0 = time.time()
        # XGBoost nécessite eval_set pour l'early stopping
        ...
        results[name] = {
            ...
            'mae': mae, 'rmse': rmse, 'r2': r2, 'train_time': t_train,
        }
        print(f"{name:<30s} {mae:>7.1f}  {rmse:>7.1f}  {r2:>7.4f}  {t_train:>6.1f}s")
 
    # --- LSTM optionnel (PyTorch) ---
    if include_lstm:
        print("\nEntraînement du LSTM PyTorch (peut prendre 3-5 minutes)...")
        try:
            import torch  # vérification que torch est disponible
            results['LSTM'] = _train_lstm(X_train_sc, y_train, X_test_sc, y_test)
            r = results['LSTM']
            print(f"{'LSTM':<30s} {r['mae']:>7.1f}  {r['rmse']:>7.1f}  "
                  f"{r['r2']:>7.4f}  {r['train_time']:>6.1f}s")
        except ImportError:
    ...
    _plot_benchmark(results, y_test, df_test['date'].values)
 
    # --- Retour des artefacts du meilleur modèle ---
    feature_ranges = {col: (X_train[col].min(), X_train[col].max()) for col in FEATURES}
    ...
    return {
        ...
        'feature_ranges':     feature_ranges,   # ← clé manquante ajoutée
    }

 
 
def _plot_benchmark(results: dict, y_test: np.ndarray, dates: np.ndarray):
    ...
    w = 0.35
    ax.bar(x - w/2, maes,  w, label='MAE',  color='steelblue', alpha=0.85)
    ax.bar(x + w/2, rmses, w, label='RMSE', color='tomato',    alpha=0.85)
    ax.set_xticks(x); ax.set_xticklabels(names, rotation=20, ha='right', fontsize=9)
    ax.set_ylabel('Erreur (Wh)')
    ax.set_title('Comparaison des erreurs (plus bas = mieux)')
    ax.legend(); ax.grid(alpha=0.3, axis='y')
 
    # --- 2. Barres R² ---
    ax = axes[0, 1]
    r2s = [results[n]['r2'] for n in names]
    colors = ['seagreen' if r == max(r2s) else 'steelblue' for r in r2s]
    bars = ax.bar(names, r2s, color=colors, alpha=0.85)
    for bar, v in zip(bars, r2s):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height(),
                f'{v:.3f}', ha='center', va='bottom', fontweight='bold')
    ax.set_xticklabels(names, rotation=20, ha='right', fontsize=9)
    ax.set_ylabel('R²')
    ax.set_title('Qualité globale R² (plus haut = mieux, vert = meilleur)')
    ax.grid(alpha=0.3, axis='y')
    ax.set_ylim(0, max(r2s) * 1.15)
 
    # --- 3. Série temporelle réelle vs prédictions ---
    ax = axes[1, 0]
    ax.plot(dates, y_test, label='Réel', color='black', linewidth=0.8, alpha=0.7)
    colors_series = plt.cm.tab10(np.linspace(0, 1, len(names)))
    for (n, color) in zip(names, colors_series):
        # On affiche juste les 3 premiers jours pour lisibilité
        ax.plot(dates[:432], results[n]['y_pred'][:432],
                label=n, linewidth=0.8, alpha=0.7, color=color)
    ax.set_title('Prédictions des modèles (premiers 3 jours du test)')
    ...
    ax = axes[1, 1]
    for (n, color) in zip(names, colors_series):
        residuals = y_test - results[n]['y_pred']
        ax.hist(residuals, bins=40, alpha=0.4, label=n, color=color)
    ax.axvline(0, color='black', linestyle='--', linewidth=1)
    ax.set_xlabel('Résidu (Wh)')
    ax.set_ylabel('Fréquence')
    ax.set_title('Distribution des erreurs par modèle\n(centré sur 0 et étroit = mieux)')
    ax.legend(fontsize=8); ax.grid(alpha=0.3)
 
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, 'model_benchmark.png')
    plt.savefig(path, dpi=130); print(f"\n-> {path}")
    plt.show()
 
 
# ============================================================
# LSTM — VERSION PYTORCH (remplace la version TensorFlow)
# Dépendances : pip install torch
# ============================================================
...
class _LSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.drop2(out[:, -1, :])   # dernier pas de temps uniquement
        out = self.relu(self.fc1(out))
        return self.fc2(out).squeeze(-1)


def _train_lstm(X_train_sc, y_train, X_test_sc, y_test, seq_len: int = 24):
    """Entraîne un LSTM simple via PyTorch.

    Même interface que la version TensorFlow :
    - prend les tableaux scalés en entrée
    - retourne dict avec model, y_pred, mae, rmse, r2, train_time
    """
    t0 = time.time()

    # --- Création des séquences ---
    def to_sequences(X, y, sl):
    ...
    X_tr_seq, y_tr_seq = to_sequences(X_train_sc, y_train, seq_len)
    X_te_seq, y_te_seq = to_sequences(X_test_sc,  y_test,  seq_len)
    ...
    model  = _LSTMModel(input_size=X_train_sc.shape[1]).to(device)
    ...
    y_pred_seq = np.concatenate(preds)

    # Rembourrage pour aligner la longueur avec y_test original
    y_pred = np.concatenate([np.full(seq_len, np.nan), y_pred_seq])

    # --- Métriques (sur les points prédits uniquement) ---
    mae  = mean_absolute_error(y_te_seq, y_pred_seq)
    rmse = np.sqrt(mean_squared_error(y_te_seq, y_pred_seq))
    r2   = r2_score(y_te_seq, y_pred_seq)

    return {
        'model': model, 'y_pred': y_pred,
        'mae': mae, 'rmse': rmse, 'r2': r2, 'train_time': time.time() - t0,
    }
